...
**The ground floor is built (2026-09-03).** `?map=g`: kitchen, pantry, mudroom, vestibule, back hall, garage, dining room, vaulted living room, half bath, hall closet, stairs and hall at honest scale, from [`docs/blueprints/floor-G.md`](docs/blueprints/floor-G.md). Two missions — **G1 The Long Hall** (string from the junk drawer, bridge the kids' marble run, climb to the second floor) and **G2 Open House** (Biscuit's ball under the dog door) — plus Tan patrols between landmarks, ambush pockets, five floor links with the toy-bin loading card, and the first secrets. Every kid-logic climb is a ramp you walk (shingled paperbacks, a leaned shelf board, the car's raked glass) except the two hard ones (fireplace stones, bookcase hops). Gates: `walk.mjs g` 13 routes clean, `house-g.mjs` 21/21, the Backyard's four gates still green; the blueprint's §11 is the as-built log of everything the gates caught. Archived as [`archive/builds/2026-09-03-ground-floor.html`](archive/builds/).

**The upper floor is standing (2026-09-04).** `?map=u` greyboxes the kids' world: Jonah's room with the loft, desk and RC track, Pip's room with **the dollhouse**, the parents' room, the bathroom with the tub, the stair head, and the hall with the attic ladder down. The floor is a **ring around the living room's vault** — there is genuinely no floor over the middle of the house, which needed a new runtime idea: `ShellDef.holes` suppress the implicit ground plane, and `killY` catches a fall and hands you back at the checkpoint. Lean over the landing rail and the living room is 46 units below, painted in as a backdrop. Walk gate: 11 routes, none failed. Population, missions, hazards and audio are still to come.

**The plan behind it:** [`docs/blueprints/floor-U.md`](docs/blueprints/floor-U.md) is the P0 contract for the kids' world — Jonah's room with the loft and the RC track, Pip's room with **the dollhouse** (the one staircase in the house built to a soldier's scale), the parents' room, the bathroom with the tub lake, and a landing whose rail looks 46 units straight down into the living room you crossed last mission. The floor is a **ring around the living room's vault**: there is no floor over the middle of the house, and that hole is the level's best idea.

**Playtest pass (2026-09-04).** First hands-on on the floor. Collision fixes: the bike and the chairs/stools were solid boxes (the marble under the bike was trapped; shots wouldn't pass under a chair) — now wheel-only and leg-only colliders. The car's window glass was raked 180° opposite its walkable surface — fixed. Launch pads are reusable. Floor transitions trimmed to **one up, one down per floor pair, plus the yard door and one express** (the garage gap and the chute's mid-stop on G are gone; the bookcase is a room climb to the overlook, not a floor change) — the confirmed plan in atlas §5. And more charm: teddy bears, alphabet blocks, a board game, a glowing lava lamp, and plastic dinosaurs, because the kids live here. Next: the second floor (`docs/10` §12 production order).
