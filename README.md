...
- **Gates in `tools/`**: `walk.mjs` (route reachability + camera sweep — the P1 gate), `combat.mjs`, `mission.mjs` (drives the whole objective chain), `smoke.mjs`, `shot.mjs`. All run headless on game-time clocks.
