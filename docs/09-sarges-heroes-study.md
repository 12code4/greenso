...
The ground floor is the first floor built ([`blueprints/floor-G.md`](blueprints/floor-G.md), `src/maps/defs/house/g.ts`): the kitchen, pantry, mudroom, vestibule, back hall, garage, dining room, vaulted living room, half bath, closet, stairs and hall at 1:32, with G1 *The Long Hall* (string from the junk drawer → bridge the marble run → the top of the stairs) and G2 *Open House* (Biscuit's ball under the dog door). Its as-built log (blueprint §11) records where the first gates caught the contract out: the staircase crossing the garage wall, half-built marble-run pieces, book stacks with jump-height risers. The rule that came out of it: **every kid-logic climb is a ramp you walk** (one-book risers, 0.3 u steps on planks and raked glass, slopes ≤ 0.77), and only the two "hard" climbs (fireplace stones, bookcase hops) ask for jumps.
