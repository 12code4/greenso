...
- **Exterior**: front door x −7.5..7.5 on z 111 (`door_closed`, knob at 18.5) with sidelight `glass` x ±9..14, y 8..37; back door x 40..55 on z −111 (`door_closed`); basement door x 30..45 on the backhall's north wall (`door_closed`, label BASEMENT, leads to B — a `link` with the not-built card); garage roll-up door x 70..130 on z −111, y 3..40 (`mass`, ribbed) with the **3 u gap** underneath (a side-quest spot, not a floor link — the trim, 2026-09-04); **dog door** x −27..−21 on z −111, y 0..5 (`dog_door_flap`, walk-through trigger `L_dogdoor`).
...
| `chute_closet` | walls at x 45..61, z −50..−31 with a door at (53, 0, −31) | the chute opening on its east wall at y 10, 6 wide; `rope_knots` up the shaft to y 46 | set dressing — the arrival shaft; **no floor link on G** (the trim): the chute is the B↔U express when those floors exist |
...
| `bookcase` ×2 | (−44.5, 0, 65.5), 29 × 37 × 6 · (45.5, 0, 65.5) | shelves 5.5 apart with **book-stack stairs** on every shelf | summit 37; `rope_knots` (45.5, 37 → 52, 62) to the balcony rail — an **in-floor climb to the overlook**, not a floor link (the trim) |
...
- **Playtest fixes (2026-09-04).** Collision: the `bike` was one solid box floor-to-bar (the marble under it was unreachable) — now just two wheel colliders, and `marble_drain` sits at (128, 0.2, −12) between them, under the crank; `chair_dining` and `stool` had a full box filling the under-seat volume (shots wouldn't pass) — now four thin leg colliders each, seat slab on top, open underneath. Orientation: the car's windshield and rear glass tilted opposite their walkable colliders (a 180° read) — the visual rake now uses `-sign * rake` to match (hood/trunk low → roof high). The `ramp_plank`s were already correct (verified by screenshot). Reuse: a `launch` interactable now resets like a `warp`, so launch pads can be pressed again. Trim: `L_garage` and `L_chute_up` removed as floor links (see the shell/rooms notes above and atlas §5). Decoration: `teddy_bear`, `toy_blocks`, `board_game`, a glowing `lava_lamp`, and `toy_dino` sprinkled through the living room, dining room, hall, mudroom, and garage — the kids' house.
- **Detail pass (2026-09-04).** Five architectural props that only read at 1:32: `outlet` (a duplex plate 30 cm up — chest height on a soldier, and the single best scale-seller on the floor), `light_switch` at 120 cm, `floor_register` (nearly flush, 0.24 u collider so you walk straight over it; the duct network of docs/11 #9 made visible), `curtain` (pleated panels on a rod at the west and stair windows), and `ceiling_light` — **off everywhere except the garage**, because it is the middle of a weekday and nobody is home. Wall props front their local −z face (the kit convention: `clock_wall`, `key_hook`), so the placement yaw is 0 → faces −z, π → +z, π/2 → −x, −π/2 → +x.
...
- **Fireplace.** The hearth is 3 u tall (a real raised hearth), so a stack of old magazines at (47.75, 0, 14) gets you onto it from the west. The stones come out of the south cheek's face in **two rows**: 8 up along the inner row (z 19..21, x 51..58.7) and 8 back along the outer row (z 21..23), **1.1 risers** on 1.1 treads, the turnaround a sideways hop — so no stone is ever above one you jump from. The first stone is a hop up from the hearth; the breast's last 1.6 u sit inside the garage wall. The top stone (20.6) sits beside the mantel (21.2), one short hop. Each stone is a 2 × 1 × 1 slab collider, not a column from the floor. What the gates taught: 1.2 risers left 0.15 u under the jump apex and made fifteen hops in a row a coin toss; a zig-zag in one row puts the turnaround stone over the stone you jump from, and a 1.35 jump under a 1.2 ceiling goes nowhere. The contract's ledges wrapped from the back straight to the front — an 11 u leap. The mantel walkway in front of the chimney breast is 1.5 u wide (8 cm — a real mantel).
